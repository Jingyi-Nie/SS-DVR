...
class EuclideanLoss:
    ...
    def compute(self, truth, pred, conf, conf_mask):
        euclidean_dist = truth - pred
        euclidean_dist = torch.norm(euclidean_dist, dim=-1)

        distance = euclidean_dist

        if conf is not None:
            loss = (conf * distance - self.alpha * torch.log(conf))
        else:
            loss = distance

        if conf_mask is not None:
            conf_mask = conf_mask[:, 0]
            loss = (loss * conf_mask).sum() / conf_mask.sum()
        else:
            loss = loss.mean()
